...
class Communicator:
    ...
    def __init__(self, port_name: str, event_loop: asyncio.AbstractEventLoop):
        """The constructor is blocking. Use the factory method new() in async contexts instead."""
        self._event_loop = event_loop
        self._ch = popcop.physical.serial_multiprocessing.Channel(
            port_name=port_name,
        ...
        self._pending_requests: typing.Set[
            typing.Tuple[typing.Callable, asyncio.Future]
        ] = set()

        self._thread_handle = threading.Thread(
            target=self._thread_entry, name="communicator_io_worker", daemon=True
        )
    ...
    @staticmethod
    async def new(
        port_name: str, event_loop: asyncio.AbstractEventLoop
    ) -> "Communicator":
        """
        Use this method to create new instances of this class from async contexts.
        """
        return await event_loop.run_in_executor(
            None, lambda: Communicator(port_name=port_name, event_loop=event_loop)
        )
            ...
            try:
                ...
                if isinstance(ret, bytes):
                    ...
                    self._event_loop.call_soon_threadsafe(
                        self._log_queue.put_nowait, (ts, log_str)
                    )
                elif ret is not None:
                    _logger.debug("Received item: %r", ret)
                    self._event_loop.call_soon_threadsafe(
                        self._process_received_item, ret
                    )
            except popcop.physical.serial_multiprocessing.ChannelClosedException as ex:
                _logger.info(
                    "Stopping the IO worker thread because the channel is closed. Error: %r",
                    ex,
                )
                break
                ...
                error_counter += 1
                _logger.exception(
                    f"Unhandled exception in IO worker thread "
    ...
    def _process_received_item(
        self, item: typing.Union[ReceivedFrame, StandardMessageBase]
    ) -> None:
        if isinstance(item, StandardMessageBase):
            ...
            if self._codec is None:
                _logger.warning(
                    "Cannot decode application-specific frame because the codec is not yet initialized: %r",
                    item,
                )
                return
            # noinspection PyBroadException
    ...
    async def _do_send(
        ...
        message_or_type: typing.Union[
            Message, StandardMessageBase, StandardMessageType
        ],
    ):
        """
        ...
        try:
            if isinstance(message_or_type, (Message, MessageType)):
                if self._codec is None:
                    raise CommunicatorException(
                        "Codec is not yet initialized, "
                ...
                raise TypeError(
                    f"Invalid message or message type {type(message_or_type)}: {message_or_type}"
                )
        except popcop.physical.serial_multiprocessing.ChannelClosedException as ex:
    ...
    def _match_message(
        reference: typing.Union[
            Message, MessageType, StandardMessageBase, StandardMessageType
        ],
        candidate: AnyMessage,
    ) -> bool:
        ...
        def super_predicate(item: AnyMessage) -> bool:
            if predicate is not None:
                try:
                    ...
                    _logger.exception(
                        "Unhandled exception in response predicate for message %r: %r",
                        message_or_type,
                        ex,
                    )
            else:
    ...
    async def close(self):
        await asyncio.gather(
            self._event_loop.run_in_executor(None, self._thread_handle.join),
...
async def _async_unittest_communicator_loopback():
    ...
    async def sender():
        ...
        with raises(CommunicatorException):
            await com.send(
                Message(
                    MessageType.COMMAND,
                    {"task_id": "hardware_test", "task_specific_command": {}},
                )
            )
    ...
    assert com.is_open
    await asyncio.gather(sender(), receiver(), log_reader(), closer())
...
async def _async_unittest_communicator_disconnect_detection():
    ...
    assert com.is_open
    await asyncio.gather(receiver(), log_reader(), closer())


def _unittest_communicator_disconnect_detection():
    asyncio.get_event_loop().run_until_complete(
        _async_unittest_communicator_disconnect_detection()
    )
