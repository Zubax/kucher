...
class MonitoredQuantityPresenter:
    ...
    def __init__(
        ...
        self._params = {
            ...
            MonitoredQuantity.Alert.TOO_LOW: params_when_low
            or self.DisplayParameters(),
            MonitoredQuantity.Alert.TOO_HIGH: params_when_high
            or self.DisplayParameters(),
        }
    ...
    def display(self, quantity: typing.Union[int, float, MonitoredQuantity]):
        ...
        self._display_target.set(
            text, style=params.style, comment=params.comment, icon_name=params.icon_name
        )
...
def _unittest_monitored_quantity_presenter():
    ...
    a = ValueDisplayWidget(
        container, "Raskolnikov", "N/A", tooltip="This is Rodion", with_comment=True
    )
    layout.addWidget(a)
    ...
    mqp = MonitoredQuantityPresenter(
        a,
        "%.1f \u00B0C",
        params_default=MonitoredQuantityPresenter.DisplayParameters(
            comment="OK", icon_name="ok"
        ),
        params_when_low=MonitoredQuantityPresenter.DisplayParameters(
            comment="Cold", icon_name="cold"
        ),
        params_when_high=MonitoredQuantityPresenter.DisplayParameters(
            comment="Hot", icon_name="fire"
        ),
    )
