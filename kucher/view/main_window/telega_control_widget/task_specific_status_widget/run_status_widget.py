...
class Widget(StatusWidgetBase):
    ...
    def reset(self):
        ...
        assert (
            num_reset > 7
        )  # Simple paranoid check that PyQt is working as I expect it to
    ...
    def on_general_status_update(self, timestamp: float, s: GeneralStatusView):
        ...
        try:
            ...
            if self._energy_conversion_efficiency_estimate is not None:
                self._energy_conversion_efficiency_estimate += (
                    eta - self._energy_conversion_efficiency_estimate
                ) * 0.2
            else:
        ...
        self._dq_display.set(tssr.u_dq, tssr.i_dq)

        cm_name, cm_icon_name = get_human_friendly_control_mode_name_and_its_icon_name(
            tssr.mode, short=True
        )
        self._control_mode_display.set(
            cm_name, comment=str(tssr.mode).split(".")[-1], icon_name=cm_icon_name
        )

        self._reverse_flag_display.set(
            "Reverse" if tssr.rotation_reversed else "Forward",
            icon_name="jog-reverse" if tssr.rotation_reversed else "jog-forward",
        )

        self._spinup_flag_display.set(
            "Starting" if tssr.spinup_in_progress else "Started",
            icon_name="warning" if tssr.spinup_in_progress else "ok-strong",
        )

        self._saturation_flag_display.set(
            "Saturated" if tssr.controller_saturated else "Normal",
            icon_name="control-saturation"
            if tssr.controller_saturated
            else "ok-strong",
        )
    ...
    def _make_display(
        self, title: str, tooltip: str, with_comment: bool = False
    ) -> ValueDisplayWidget:
        return ValueDisplayWidget(
            self, title=title, with_comment=with_comment, tooltip=tooltip
        )
...
class _DQDisplayWidget(QWidget):
    ...
    def set(self, u_dq: typing.Tuple[float, float], i_dq: typing.Tuple[float, float]):
        def fmt(x: float) -> str:
            return f"{x:.1f}"
